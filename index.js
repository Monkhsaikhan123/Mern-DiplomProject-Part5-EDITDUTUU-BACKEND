...
app.delete('/menu2/:id', async(req,res)=>{
    const id = req.params.id;
    console.log(id)
    const data = await Products2.deleteOne({_id: id, email:email})
    res.send({success:true, message: "Product has Deleted", data:data})
})
...
app.get('/like', async(req,res)=>{
    let r = await Like.find({})
    console.log(r)
    res.send(r);
})
app.post('/like', async(req,res)=>{
    const {blogItemId, recipe, name, description, image, price, quantity, email, phoneNumber, category} = req.body;
    try {
       //existing menu item
 /*     const existingCartItem = await Cart.findOne({menuItemId})
       if(existingCartItem){
       ...
       const LikeItem = await Like.create({blogItemId, name, recipe, image, price, quantity, category, phoneNumber,description,email})
